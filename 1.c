/**************************************************************************************************/
/* Copyright (C) mc2lab.com, SSE@USTC, 2014-2015                                                  */
/*                                                                                                */
/*  FILE NAME             :  menu.c                                                               */
/*  PRINCIPAL AUTHOR      :  Luoyukun                                                             */
/*  SUBSYSTEM NAME        :  menu                                                                 */
/*  MODULE NAME           :  menu                                                                 */
/*  LANGUAGE              :  C                                                                    */
/*  TARGET ENVIRONMENT    :  ANY                                                                  */
/*  DATE OF FIRST RELEASE :  2014/09/14                                                           */
/*  DESCRIPTION           :  This is a menu program                                               */
/**************************************************************************************************/

/*
 * Revision log:
 *
 * Created by Luoyukun, 2014/09/14
 *
 */

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define DESC_LEN    1024
#define CMD_NUM     10

typedef struct DataNode
{
    char    cmd[DESC_LEN];
    char    desc[DESC_LEN];
    struct  DataNode *next;
} tDataNode;

/*check the cmd repeat or not*/
int Check(char* ,tDataNode *);

 /* Init cmd list */
tDataNode* Init(tDataNode *);


/*show cmd list*/
void ShowList(tDataNode *);


/* cmd line begins */
void CmdLineBegins(tDataNode *);


 main()
{
    tDataNode * head= NULL;
    tDataNode * p = NULL;
	head = Init(head);  /* Init cmd list */
    printf("Menu List:\n");
    p = head;
	ShowList(p);        /*show cmd list*/              
	CmdLineBegins(head);   /* cmd line begins */
}



int Check(char* cmd,tDataNode *head)
{
	 tDataNode * p = NULL;
	 p = head;
	while(p != NULL)
	{
		if(strcmp(cmd,p->cmd) == 0)
		{
			printf("repeat!please reinput\n");
			return 1;
		}
		p = p->next;
	}
	return 0;
}




tDataNode* Init(tDataNode *head)
{
	int i;
	tDataNode * p = NULL;
	 for (i=0; i<CMD_NUM; i++)
    {
        p = (tDataNode*)malloc(sizeof(tDataNode));
       
        printf("please input %d cmd\n",i+1);
		scanf("%s",p->cmd);
		while( Check(p->cmd,head) == 1)
		{
			printf("please reinput %d cmd\n",i+1);
			scanf("%s",p->cmd);
		}
       
        snprintf(p->desc, DESC_LEN, "This is %s cmd!", p->cmd);
        p->next = head;
        head = p;
    }
	 return head;
}



void ShowList(tDataNode *p)
{
	 while(p != NULL)
    {
        printf("%s - %s\n", p->cmd, p->desc);
        p = p->next;
    }
}



void CmdLineBegins(tDataNode *head)
{
	tDataNode *p;
	while(1)
    {
        char cmd[DESC_LEN];
        printf("Input a cmd > ");
		scanf("%s",cmd);
  
        p = head;
        while(p != NULL)
        {
            if(strcmp(cmd,p->cmd) != 0)
            {
                p = p->next;
                continue;
            }
            else
            {
                break;
            }
        }
        
        if(!p)
        {
            printf("This is a wrong cmd !\n ");
            continue;
        }
        
        p = head;
        while(p != NULL)
        {
            if(strcmp(cmd,p->cmd) == 0)
            {
                printf("%s - %s\n", p->cmd, p->desc);
                break;
            }
            p = p->next;
        }
    }
}


